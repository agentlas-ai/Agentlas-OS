...
- `{{ARGS}}` ## Core-owned Goal + UltraCode harness Every result includes `execution_harness`.
...
- FABRIC="$("$RUNNER" hep-storm "{{ARGS}}" --research-evidence --runtime claude-code)" printf '%s\n' "$FABRIC" ``` 2.
...
- `{{ARGS}}` Codex plugins cannot register slash commands, so this custom prompt is the explicit entrypoint (`/prompts:hep-storm`).
...
- ## Examples ```text /prompts:hep-storm ship a working waitlist landing page with a verified signup flow /prompts:hep-storm 이 리포 결제 버그를 재현 PoC까지 만들어서 고치고 회귀 테스트로 검증해줘 @Hephaestus storm turn this research question into a cited report with evidence ```
...
- Pass live session JSON with `AGENTLAS_SESSION_INVENTORY` when available; otherwise use Core's explicit `host:primary` fallback.
...
- Only if the goal is too ambiguous to decompose safely, ask ONE batch of 3–5 questions (what NOT to do / smallest version / done signal / dependencies), then proceed.
...
- # Hephaestus Stormbreaker loop Drive a goal through the **Stormbreaker Loop** — Hephaestus' force-robust, verifier-first execution loop.
- Resolve the runner — first executable wins:
- FABRIC="$("$RUNNER" hep-storm "{{ARGS}}" --research-evidence --runtime opencode)" printf '%s\n' "$FABRIC" ``` Read `route_decision.action` (or `route_action`) and branch — Stormbreaker only auto-materializes a full fabric for a **pipeline**; other actions still start a storm, just with the workforce the router chose:
