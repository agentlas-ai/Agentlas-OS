"""Optional Steel remote browser snapshot hardpoint."""
...
class SteelBrowserAdapter(CommandSnapshotAdapter):
    module_id = "browser.steel"
    capabilities = ("browser.remote_session", "browser.snapshot", "read.url")
    weight = "browser_heavy"
    env_var = "AGENTLAS_STEEL_SNAPSHOT_CMD"
    command_label = "steel_snapshot"
    output_fields = ("content_markdown", "snapshot", "result", "text", "stdout")
    base_limits = ("browser_snapshot", "steel_remote_browser")
    missing_reason = "AGENTLAS_STEEL_SNAPSHOT_CMD not configured"
    manifest = ResearchModuleManifest(
        ...
        requires=["command:steel-snapshot"],
        permissions=["local_process:steel_snapshot", "network:http", "network:https", "browser:remote_session"],
        ...
        install_hint=(
            "Set AGENTLAS_STEEL_SNAPSHOT_CMD to a local Steel remote-browser snapshot command. "
