...
`curl -fsSL https://raw.githubusercontent.com/agentlas-ai/Hephaestus/v0.4.6/scripts/install-all-runtimes.sh | bash`
...
```bash
codex plugin marketplace add agentlas-ai/Hephaestus --ref v0.4.6
...
```bash
curl -fsSL https://raw.githubusercontent.com/agentlas-ai/Hephaestus/v0.4.6/scripts/install.sh | bash
...
```powershell
$zip = "$env:TEMP\hephaestus-v0.4.6.zip"
$extract = "$env:TEMP\hephaestus-v0.4.6"
Invoke-WebRequest "https://github.com/agentlas-ai/Hephaestus/archive/refs/tags/v0.4.6.zip" -OutFile $zip
...
**New in v0.4.6 — Hub invocation through Hephaestus Network MCP:**
