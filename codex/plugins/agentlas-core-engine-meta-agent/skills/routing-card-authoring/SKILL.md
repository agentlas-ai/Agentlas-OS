...
### The workforce block — semantic résumé fields

`workforce` is defined in `schemas/routing-card.schema.json` and is what the hub reads. Use ONLY ids from
the pinned ontology `agentlas_cloud/workforce/ontology_v1.json`
(`awo:2026-07-15.2`). An invented id is silently unmatchable.

| Field | Hub use | What to write |
|---|---|---|
| `workforce.communities` | semantic scope + strongest ranking weight | 1–3 `community:*` ids from the published ontology. |
