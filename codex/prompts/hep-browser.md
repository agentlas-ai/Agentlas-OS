...
- Run the installer first." >&2; exit 1; } "$RUNNER" hep-browser "$ARGUMENTS" ``` Report whether `browser.agent_cli` mounted, whether setup is needed, and the receipt id.
...
- Run the installer first." >&2; exit 1; } "$RUNNER" hep-browser "$ARGUMENTS" ``` Use this for rendered/browser-required work and report the `browser.agent_cli` status plus receipt id.
