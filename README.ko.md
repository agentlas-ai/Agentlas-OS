...
- **Hub 분류 정리.** 공개 Hub에서는 Agent, Team, Plugin만 구분하고, 호출
  크레딧은 다운로드 가능한 패키지와 별도로 표시합니다.
- **로컬 trust는 로컬 trust.** local routing-card의 `trusted`는 전역
  local-first Network가 부를 때 쓰는 신뢰 경로이고, upload/Hub publish
  보안 심사는 별도 gate로 유지합니다.
- **개인 namespace 비공개.** Mason Mac에서 쓰는 로컬 package bucket 이름은
  docs, adapter skill, CLI tier, benchmark, runtime mirror에 노출되지 않게
  정리했습니다.
