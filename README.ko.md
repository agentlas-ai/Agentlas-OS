...
- **`/hep-upload`가 자체 동작.** Cloud/Hub 업로드가 더 이상 특정 개인 checkout
  이나 외부 publish script를 요구하지 않고, bundled Hephaestus
  package/publish runtime을 사용합니다.
- **Hub 업로드 gate 내장.** marketplace 업로드 전에 `publicProfile`,
  `routing-card/2.0`, package hash, static security, bundle size를 검증합니다.
- **routing-card migration 수정.** 자동 생성 카드가 null package hash를 내보내지
  않고, bundled meta-agent card도 `routing_ready` 상태로 복구됐습니다.
