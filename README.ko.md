...
기존 `/hephaestus`도 build alias로 계속 동작하지만, 새 문서와 설치 안내는 위
세 이름을 기준으로 갑니다.
...
- **명령은 세 개로 압축.** `/hephaestus-build`는 생성,
  ...
  Agentlas Cloud에 공유/보관한 에이전트 사용입니다. 기존 `/hephaestus`는
  build 호환 alias로 계속 동작합니다. `ao`, `network`, `cards`, `mcp`는
  자동화와 디버그용 내부 표면으로 남습니다.
...
`/hephaestus ontology`는 이제 메인 Agent OS 화면이 아니라 Knowledge/Memory 패널입니다. 프로젝트 소스, memory candidate, 재사용 가능한 playbook 재료를 확인할 때 씁니다. 왼쪽 네비게이션, Obsidian 스타일 지식 그래프, 소스 검색, GraphRAG 질문 빌더, Memory Candidate Queue, 복사 가능한 명령어 화면이 포함됩니다.
...
기존 `/hephaestus`는 build alias로 계속 동작합니다.
