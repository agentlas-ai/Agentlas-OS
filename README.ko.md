...
- **빌더 인터뷰가 필수가 됐습니다.** `/hephaestus-build`는 단일 에이전트,
  멀티 에이전트 팀, 패키징 결과를 본격 생성하기 전에 8-12개 핵심 질문을 먼저
  해야 합니다. 핵심 답이 부족하면 generic agent를 만들지 않고
  `needs_clarification`을 반환합니다.
- **유사 에이전트와 repository 리서치를 요구합니다.** 공개 또는 marketplace용
  빌드는 최소 3개의 유사 agent, repo, system, example, benchmark를 비교해야
  하며, 직접 비교 대상이 없으면 검색어와 가장 가까운 analog를 기록해야 합니다.
- **학술/전문 이론이 프롬프트 계약에 들어갑니다.** baseline agent-design
  research와 도메인별 academic, standard, legal, professional source를
  prompt behavior로 반영합니다.
- **Domain expert synthesis가 생겼습니다.** 생성 패키지는
  `docs/domain-expert-synthesis.md`에 인터뷰 답변, repo pattern, 이론,
  tool/plugin 결정을 합쳐 specialist heuristic, 예시, 반례, eval case로
  변환합니다.
- **검증이 퇴행을 잡습니다.** `scripts/verify-builder-quality-contract.sh`가
  package verification에 포함되어 런타임 adapter가 인터뷰/리서치/이론/합성
  게이트를 빼먹으면 실패합니다.
