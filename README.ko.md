...
hephaests-network "이 일을 임시 TF로 쪼개줘"
```
...
- **명령은 두 개로 압축.** 사람은 대부분 `/hephaestus-network` 또는
  `hephaests-network`로 작업을 맡기고, `/hephaestus` 또는 `Hephaestus`로
  생성·수리·메모리·플레이북 관리를 요청하면 됩니다. `ao`, `network`,
