...
- **Agentlas 네이티브, 외부 LLM 여섯 명령.** Agentlas Terminal과 Agentlas 앱은
  자연어 네이티브 표면입니다. 하고 싶은 일을 말하면 Agentlas/Hephaestus 도구가
  맥락에 맞게 경로를 고릅니다. Claude Code, Codex, Gemini CLI, Antigravity,
  Cursor, OpenCode 같은 외부 LLM 호스트는 `/hep-build`, `/hep-network`,
  `/hep-cloud`, `/hep-search`, `/hep-call`, `/hep-upload` 여섯 명령만 보입니다.
- **Research Engine phase-0 core.** Hephaestus에 public-safe 리서치 엔진이
  들어갔습니다. `auto`, `safe`, `public-web`, `social`, `browser`, `full`,
  `recommended` loadout과 built-in cartridge, SSRF-safe reader, receipt ledger,
  plan/gather/search/read/status/proofs/verify/credentials/hardpoints CLI 흐름을
  제공합니다.
- **insane-search는 전체 엔진이 아니라 cartridge.** `read.insane_fetch` reader는
  `public-web`, `social`, `browser`, `full` 같은 무거운 loadout이나 명시 allow
  list에서만 붙습니다. 직접 읽기, Reddit RSS, Jina Reader fallback, metadata/feed
  parsing, login/paywall stop 증거를 남기지만 기본 리서치 두뇌가 되지는 않습니다.
- **Stormbreaker research evidence.** Stormbreaker packet이 research receipt,
  preflight file, readiness snapshot, capability summary, evidence-quality/coverage
  신호를 붙일 수 있습니다. `recommended` loadout은 원래 사용자 요청을 보고 packet
  별로 결정되어, 공개 웹/소셜 리서치가 필요할 때만 `public-web` 쪽을 고릅니다.
