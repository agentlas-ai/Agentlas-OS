...
How `/hephaestus-build`, `/hephaestus-network`, and `/hephaestus-cloud` get registered per runtime, and
...
| Terminal | automatic | `bin/hephaestus-build`, `bin/hephaests-network`, and `bin/hephaestus` — `Hephaestus-build "<request>"` builds, `hephaests-network "<request>"` borrows Hub agents, `hephaestus cloud "<request>"` uses the signed-in user's cloud packages |
