...
`curl -fsSL https://raw.githubusercontent.com/agentlas-ai/Hephaestus/v0.7.17/scripts/install-all-runtimes.sh | bash`
...
```bash
curl -fsSL https://raw.githubusercontent.com/agentlas-ai/Hephaestus/v0.7.17/scripts/install.sh | bash
...
```powershell
$zip = "$env:TEMP\hephaestus-v0.7.17.zip"
$extract = "$env:TEMP\hephaestus-v0.7.17"
Invoke-WebRequest "https://github.com/agentlas-ai/Hephaestus/archive/refs/tags/v0.7.17.zip" -OutFile $zip
...
```bash
codex plugin marketplace add agentlas-ai/Hephaestus --ref v0.7.17
codex plugin list
