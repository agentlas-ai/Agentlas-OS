...
- **Builder interview 现在是强制门禁。** `/hephaestus-build` 在生成重要的 single-agent、team 或 packaging 输出前，必须先问 8-12 个高价值问题。
- **必须研究相似 agent 和 repository。** 面向公开或 marketplace 的构建必须比较至少 3 个相似 agent、repo、system、example 或 benchmark；如果没有直接可比对象，必须记录搜索词和最近似的 analog。
- **学术或专业理论进入 prompt contract。** 构建过程会使用 baseline agent-design research，并在可用时加入领域内 academic、standard、legal 或 professional source。
- **新增 domain-expert synthesis。** 生成包会写入 `docs/domain-expert-synthesis.md`，把访谈、repo pattern、理论和 tool/plugin 选择转成 specialist heuristics、examples、counterexamples 和 eval cases。
- **验证会防止回退。** `scripts/verify-builder-quality-contract.sh` 已接入 package verification；runtime adapter 如果漏掉 interview/research/theory/synthesis gate 会失败。
