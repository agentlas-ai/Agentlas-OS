...
- **Builder interview が必須になりました。** `/hephaestus-build` は重要な single-agent、team、packaging 出力を書く前に、8-12 個の高レバレッジ質問を先に行います。
- **類似 agent / repository research を要求します。** public / marketplace-ready な build は、少なくとも 3 つの類似 agent、repo、system、example、benchmark を比較します。直接の比較対象がなければ検索語と近い analog を記録します。
- **学術・専門理論が prompt contract に入ります。** baseline agent-design research に加え、利用可能な domain-specific academic、standard、legal、professional source を反映します。
- **Domain-expert synthesis を追加。** 生成 package は `docs/domain-expert-synthesis.md` に interview、repo pattern、theory、tool/plugin choices をまとめ、specialist heuristics、examples、counterexamples、eval cases に変換します。
- **検証で後退を検出。** `scripts/verify-builder-quality-contract.sh` が package verification に入り、runtime adapter が interview/research/theory/synthesis gate を落とすと失敗します。
