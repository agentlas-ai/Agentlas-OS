...
`ontology`, first run the app-host auto-update preflight from that skill inside
OpenCode, then run `"$RUNNER" ontology`. Do not ask the user to open a separate
terminal; if the preflight cannot install, continue with the installed runner or
report `/tmp/hephaestus-app-auto-update.log`. Otherwise classify the request as
