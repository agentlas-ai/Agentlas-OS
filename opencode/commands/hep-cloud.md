...
`{workOrder, sourceScope: "cloud"}`. Retain the projected menu's
...
synthesis, and verifier while retaining source `cloud` and every immutable
pin. For `partial` or `failed`, report each source receipt's exact `failureCode`;
never collapse, substitute, or relabel it. Never search Local or public Hub, use legacy routing, accept deterministic
staffing, or claim execution from preparation.
